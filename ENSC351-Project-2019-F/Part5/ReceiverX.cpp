...
goodBlk. This variable will be made false if either
	ï¿½ the needed number of bytes have not yet been received and another
	...
	ï¿½ the needed number of bytes (or more) are received and the block
	created using the needed number of bytes has something
	wrong with it, like the checksum being incorrect.
	ï¿½ more than the needed number of bytes are received.
...
void ReceiverX::clearCan()
{
	...
	while (1){
		int numBytesRead = myReadcond(mediumD, &byteReceived, 1, 1, TM_CHAR, TM_CHAR);
		if(byteReceived != CAN)
		{
			CON_OUT(consoleInId, byteReceived << flush);
			break;
		}
		else if(numBytesRead == 0 || numBytesRead == -1)
		{
			break;
		}
	}
...
{
    char bytesToBeReceived = CAN;
    while (1)
    {
        int ReadBytes = myReadcond(mediumD, &bytesToBeReceived, 1, 1, TM_2CHAR, TM_2CHAR);
        if(ReadBytes == 0)
        {
            break;
        }
